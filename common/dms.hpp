...
namespace hu
{
    ...
    public:
        ...
        DMSClient(const std::string &access_key_id,
            ...
            _client = std::make_unique<AlibabaCloud::CommonClient>(credential, configuration);//构造客户端
        }
        ...
        bool send(const std::string &phone, const std::string &code) {
            ...
            request.setQueryParameter("SignName", "bitejiuyeke");//need change 
            ...
            auto response = _client->commonResponse(request);//通过client 通过 request进行短信的发送
            if (!response.isSuccess()) {
                LOG_ERROR("code request failed {}", response.error().errorMessage());
                return false;
            }
            return true;
        }
